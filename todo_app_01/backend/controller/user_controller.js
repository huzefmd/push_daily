...
const user_schema=z.object({
    ...
    username:z.string().min(5,{msg:"username should be more than 5 charecters"}),
    password:z.string().min(6,{msg:"password should be more than 6 charecters"}),
});
...
export const remove_user=async(req,res)=>{
    try {
        ...
        res.status(200).json({msg:"Suucesfully all User in the Data base are removed"})
...
export const fetch_user=async(req,res)=>{
    try {
        ...
        res.status(200).json({msg:"User Data Fetched Suucesfully From the Data base",allUser})
    } catch (error) {
        console.log(error)
        res.status(400).json({error:"Error in fetchging User Data from the data base"})
